...
> 
> 1. 柯里化：一个函数原本有多个参数，只传入**一个**参数，生成一个新函数，由新函数接收剩下的参数来运行得到结果。
> 2. 偏函数：一个函数原本有多个参数，只传入**一部分**参数，生成一个新函数，由新函数接收剩下的参数来运行得到结果。
> 3. 高阶函数：
> 	1. 定义(满足下面的一个条件即可)
> 		1. 若 A 函数，接收到的参数是一个函数，那么 Ａ 就可以被称为高阶函数
> 		2. 若 A 函数，调用的返回值仍然是一个函数，那么 A 就可以被称为高阶函数
...
-  **缓存一部分行为(vue中AST的缓存)** - AST被保存在不销毁的私有作用域里，即缓存，之后返回的函数都可以使用这个缓存的AST
...
![[../../../⚡框架/vue/img/Pasted image 20220515231835.png]]

本例实际上用的是 参数的复用

**难点解释**：name_mid为柯里化后的函数，nameList1调用map遍历每个对象，因此name_mid就自动接收了每个被遍历到的对象，即element参数。


==***在react中的应用***==

[[../../../⚡框架/React/组件实例三大属性/state/收集数据/01柯里化收集|收集数据于state中]]